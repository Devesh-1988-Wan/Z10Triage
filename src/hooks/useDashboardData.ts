...
export const useDashboardData = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let dbLayoutData: any = null; // To store the full layout record
      ...
      if (dbLayoutData) {
        setDashboardId(dbLayoutData.id);
        setDashboardName(dbLayoutData.dashboard_name || 'Z10 Updates');
        setDashboardDescription(dbLayoutData.dashboard_description || 'Development progress, bug tracking, and customer support overview');
        setDashboardLayout(dbLayoutData.layout as unknown as DashboardLayout);
      } else {
        // Fallback to the hardcoded default layout if nothing is in the database
        setDashboardName('Z10 Updates');
        setDashboardDescription('Development progress, bug tracking, and customer support overview');
