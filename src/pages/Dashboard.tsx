...
from '@/components/DateRangePicker'; // Corrected import path and component name
import {
  AdminForms
}
...
const Dashboard: React.FC = () => {
      ...
      if (dashboardLayout) {
          const updatedWidgets = dashboardLayout.widgets.map(widget => {
              ...
              return layoutItem ? { ...widget,
                  layout: { ...widget.layout, ...layoutItem
                  }
              } : widget;
          });
  ...
  if (error) {
      return <div className = "text-destructive text-center p-4" > {
          error
      } < /div>;
  }
  ...
  return ( <
      ...
      {
          canEdit && < AdminForms onDataUpdate = {
              refetch
          }
          bugReports = {
              bugReports
